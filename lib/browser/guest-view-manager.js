...
const assertWebViewTagEnabled = function (contents) {
  if (!isWebViewTagEnabled(contents)) {
    throw new Error('<webview> disabled')
  }
}

ipcMainUtils.handle('ELECTRON_GUEST_VIEW_MANAGER_CREATE_GUEST', function (event, params) {
  assertWebViewTagEnabled(event.sender)
...
ipcMainUtils.handle('ELECTRON_GUEST_VIEW_MANAGER_CALL', function (event, guestInstanceId, method, args) {
  assertWebViewTagEnabled(event.sender)
